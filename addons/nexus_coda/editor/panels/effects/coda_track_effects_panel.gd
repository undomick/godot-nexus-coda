@tool
class_name CodaTrackEffectsPanel
...
func _ready() -> void:
	...
	_empty = Label.new()
	_empty.text = "Pick a track in the Timeline panel."
...
func attach_timeline_panel(panel: CodaTimelinePanel) -> void:
	if _timeline != null and is_instance_valid(_timeline):
		if _timeline.track_selection_changed.is_connected(_on_track_selection_changed):
			_timeline.track_selection_changed.disconnect(_on_track_selection_changed)
	_timeline = panel
	if _timeline != null:
		_timeline.track_selection_changed.connect(_on_track_selection_changed)
	_refresh()


func focus_current_track() -> void:
	_refresh()


func _on_track_selection_changed(event_id: String, track_id: String) -> void:
	_event_id = event_id
	_track_id = track_id
	_refresh()
...
func _resolve_track() -> CodaTimelineTrack:
	if _project == null or _event_id.is_empty() or _track_id.is_empty():
		return null
	...
	if node.event_timeline == null:
		return null
	return node.event_timeline.find_track(_track_id)


func _refresh() -> void:
	var tr: CodaTimelineTrack = _resolve_track()
	if tr == null or _track_id.is_empty():
		_empty.visible = true
		_chain.visible = false
		return
	_empty.visible = false
	_chain.visible = true
	_chain.set_chain_title("Track: %s" % tr.track_name)
	_chain.bind_effects_array(tr.effects)


func _on_chain_add(effect_type: int) -> void:
	if _project == null:
		return
	_project.add_track_effect(_event_id, _track_id, effect_type as CodaTrackEffect.Type)


func _on_chain_remove(effect_id: String) -> void:
	if _project == null:
		return
	_project.remove_track_effect(_event_id, _track_id, effect_id)


func _on_chain_move(from_i: int, to_i: int) -> void:
	if _project == null:
		return
	_project.move_track_effect(_event_id, _track_id, from_i, to_i)
...
func _on_chain_param(effect_id: String, key: String, value: float) -> void:
	...
	_project.set_track_effect_params(_event_id, _track_id, effect_id, d)


func _on_chain_bypass(effect_id: String, on: bool) -> void:
	if _project == null:
		return
	_project.set_track_effect_bypass(_event_id, _track_id, effect_id, on)
