...
static func plan(graph: CodaEventGraph, param_values: Dictionary = {}, seed: int = 0) -> Array:
	var out: Array = []
	if graph == null:
		return out
	...
	if seed != 0:
		...
		rng.randomize()
	_walk(graph, trigger, rng, param_values, out)
	return out
	...
	match node.kind:
		NodeData.Kind.TRIGGER:
			for child in graph.get_children(node.id):
				_walk(graph, child, rng, param_values, out)
		...
		NodeData.Kind.RANDOM:
			...
			_walk(graph, chosen, rng, param_values, out)
		NodeData.Kind.SWITCH:
			_walk_switch(graph, node, rng, param_values, out)
		NodeData.Kind.BLEND:
			_walk_blend(graph, node, rng, param_values, out)
		NodeData.Kind.SOUND:
...
static func _walk_switch(
	...
	_walk(graph, children[idx], rng, param_values, out)
...
static func _walk_blend(
	...
	if children.is_empty():
		return
	if children.size() == 1:
		_walk(graph, children[0], rng, param_values, out)
		return
	var param_id: String = String(node.properties.get("parameter_id", ""))
	var t: float = 0.0
	if not param_id.is_empty() and param_values.has(param_id):
