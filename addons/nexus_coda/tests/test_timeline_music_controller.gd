...
static func run() -> int:
	...
	failed += _test_marker_at_timeline_origin()
...
static func _test_marker_at_timeline_origin() -> int:
	var markers: Array[String] = []
	var ctrl := CodaTimelineMusicControllerScript.new()
	...
	var origin_marker := CodaTimelineMarkerScript.new()
	origin_marker.time_seconds = 0.0
	timeline.markers.append(origin_marker)
	var handle: CodaEventHandle = CodaEventHandleScript.new()
	handle.is_timeline = true
	handle.event_node = ev
	var dispatchers: Dictionary = {handle: {"timeline": timeline, "fired_marker_ids": {}}}
	ctrl.check_markers_crossed(handle, timeline, 0.0, 0.05, dispatchers)
	if markers.size() != 1 or markers[0] != origin_marker.id:
		push_error("marker at t=0 should fire when playback starts at the origin")
		return 1
	markers.clear()
	ctrl.check_markers_crossed(handle, timeline, 0.05, 0.1, dispatchers)
	if not markers.is_empty():
		push_error("origin marker must not fire again on later ticks")
		return 1
	return 0
